            ...
            <div key={order._id} className='rounded-xl border border-pink-100 bg-white p-5 shadow-sm'>
              ...
              {order.user && (
                <div className='mt-3 text-sm text-slate-600'>
                  <p onClick={()=>navigate(`/dashboard/users/${order.user._id}`)}><span className='font-medium text-slate-700'>User:</span> {order.user.firstName} {order.user.lastName}</p>
                  <p onClick={()=>navigate(`/dashboard/users/${order.user._id}`)}><span className='font-medium text-slate-700'>Email:</span> {order.user.email}</p>
                </div>
              )}
